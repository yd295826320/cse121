...
#define HIDD_DEVICE_NAME            "Duooooooo"
...
void hid_demo_task(void *pvParameters)
{
    ...
    */
    ...
    while (1) {
        ...
        if (sec_conn) {
		/*
		ESP_LOGI(HID_DEMO_TAG, "Moving right");
		for (int i = 0; i < 800;i++){
			vTaskDelay(10 / portTICK_PERIOD_MS);
			esp_hidd_send_mouse_value(hid_conn_id, 0, 1, 0);
        	}
		vTaskDelay(5000 / portTICK_PERIOD_MS); // Pause for 5 seconds
		ESP_LOGI(HID_DEMO_TAG, "Moving left");
		for (int j = 0; j < 800;j++){
			vTaskDelay(10 / portTICK_PERIOD_MS);
			esp_hidd_send_mouse_value(hid_conn_id, 0, -1, 0);
    		}
		...
		*/
		...
		int16_t accel_y = (int16_t)((read(Y1) << 8) | read(Y0));
        	ESP_LOGI(HID_DEMO_TAG, "Acceleration X: %i, Y: %i", accel_x, accel_y);
	
		int16_t move_x = 0;
		int16_t move_y = 0;
		if (accel_x > 200 || accel_x < -200){
			move_x = 0 - ((accel_x / 100) - 2);
		}
		else{
			move_x = 0;
		}	
		if (accel_y > 200 || accel_y < -200){
			move_y = 0 - ((accel_y / 100) - 2);
		}
		else{
			move_y = 0;
		}
        	ESP_LOGI(HID_DEMO_TAG, "Moving X: %i, Y: %i", move_x, move_y);

		esp_hidd_send_mouse_value(hid_conn_id, 0, move_x, move_y);
